...
def consistent(assignment, csp, var, value):
	...
	for x in csp.binaryConstraints:
		if x.affects(var): # Constraint has impact on var
			otherVar = x.otherVariable(var) # the other variable affected
			if(value == assignment.assignedValues[otherVar]): # values already assigned for the other value
				return False
	return True
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod):
	...
	if assignment.isComplete():
		return assignment
	var = minimumRemainingValuesHeuristic(assignment,csp)
...
def minimumRemainingValuesHeuristic(assignment, csp):
	...
	for d in domains:
		if len(domains[d])<=minDomain and not assignment.isAssigned(d):
			oldMin = minDomain
			minDomain=len(domains[d])
			oldVar = currentVar
			currentVar = d
			nextVar = d
			if(minDomain==oldMin and not assignment.isAssigned(oldVar) and oldVar is not None):
				nextVarTemp = degreeTie(assignment, csp, oldVar, currentVar)
				if(maxDegree is None):
					maxDegree = nextVarTemp[1]
					maxDegreeVar = nextVarTemp[0]
					nextVar = maxDegreeVar
				if(nextVarTemp[1]>=maxDegree):
					nextVar = nextVarTemp[0]
					maxDegree = nextVarTemp[1]
					maxDegreeVar = nextVarTemp[0]
				else:
					nextVar = maxDegreeVar
...
def degreeTie(assignment, csp, var1, var2):
	var1Affect = 0
	var2Affect = 0
	for x in csp.binaryConstraints:
		if x.affects(var1):
		...
		if x.affects(var2):
			var2Affect+=1
	if(var1Affect>=var2Affect):
		return [var1,var1Affect]
	return [var2,var2Affect]
...
def countConstrainedValue(assignment, csp, var, varDomain):
	countConstrain = {} # Dictionary that contains the varDomain and count
	for v in varDomain:
		countConstrain[v]=0
	neighborVars =  []
	for x in csp.binaryConstraints:
		if x.affects(var):
			neighborVars.append(x.otherVariable(var))
	for neighbor in neighborVars:
		neighborDomain = assignment.varDomains[neighbor]
		for domain in neighborDomain:
			if domain in countConstrain:
				oldValue = countConstrain[domain]
				countConstrain[domain] = oldValue+1
	sorted_x = sorted(countConstrain.items(), key=operator.itemgetter(1))
	return sorted_x
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	"""Question 3"""
	counstrainResult = countConstrainedValue(assignment, csp, var, values)
	rlist = []
	for itemX in counstrainResult:
		rlist.append(itemX[0])
	return rlist
...
def forwardChecking(assignment, csp, var, value):
	...
	"""Question 4"""
	for x in csp.binaryConstraints:
		if(x.affects(var) and assignment.assignedValues[x.otherVariable(var)]==None):
			otherVar = x.otherVariable(var)
			if value in domains[otherVar]:
				domains[otherVar].remove(value)
				inferences.add((otherVar,value))
				neighbor.append(otherVar)
	for n in neighbor:
		if(len(domains[n])==0):
			isNone = True
	if isNone:
...
def recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
	...
	for values in orderValuesMethod(assignment,csp,variable):
		if consistent(assignment,csp,variable,values):
			assignment.assignedValues[variable]=values
			inferences = inferenceMethod(assignment,csp,variable,values)
			if inferences != None:
				for x,y in inferences:
...
def revise(assignment, csp, var1, var2, constraint):
	...
	for y in domains[var2]:
		...
		if satisfy == False:
			remove.append(y)
	for item in remove:
		domains[var2].remove(item)
		inferences.add((var2, item))
	return inferences
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	for c in csp.binaryConstraints:
		if c.affects(var):
			...
			if assignment.assignedValues[otherVar]==None:
				q.put((var, otherVar, c))
				q.put((otherVar, var, c))
	while(not q.empty()):
	 	v1, v2, c = q.get()	
		current = revise(assignment, csp, v1, v2, c)
		if current is not None:
			if(len(current) != 0):
				inferences = inferences.union(current)	
				for c in csp.binaryConstraints:
					otherVariable = c.otherVariable(v2)
					if (c.affects(v2) and assignment.assignedValues[otherVariable] == None):
						q.put((v2, otherVariable, c))	
		else:
			for x,y in inferences:
				domains[x].add(y)
			return None		
...
def AC3(assignment, csp):
	...
	domains = assignment.varDomains			
	q = Queue.Queue()
	for v1 in csp.varDomains.keys():
		for constraint in csp.binaryConstraints:
			if(constraint.affects(v1)):
				v2 = constraint.otherVariable(v1)
				q.put((v1,v2,constraint))
	while(not q.empty()):
	 	v1, v2, c = q.get()
		current = revise(assignment, csp, v1, v2, c)
		if current is not None:
